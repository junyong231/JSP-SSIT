...
<script type="text/javascript"> //스크립트로 상태관리,, 서버에서도 할 줄 알아야 함.

let url = location.href;
//alert(url) //http://localhost/jspPro/days04/empsearch.htm?deptno=30&job=MANAGER

$(":checkbox[name=deptno]").each(function(index, elem){
	
	let cboxValue = $(elem).val();
	if ( url.indexOf(`deptno=\${cboxValue}`) != -1 ) { //있다면 (인덱스 위치값 != -1)
		$(elem).prop("checked" , true)
	}
	
});		

$(":checkbox[name=job]").each(function(index, elem){
	
	let cboxValue = $(elem).val();
	if ( url.indexOf(`job=\${cboxValue}`) != -1 ) { //있다면
		$(elem).prop("checked" , true)
	}
