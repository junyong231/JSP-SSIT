...
<table>
	...
	<tbody>
		...
		<tr>
			...
			<!-- 아이디 또는 val에 동적으로 empno 숨기기!  -->
			 <%-- data-XXX=<%= evo.getEmpno() %> data-YYY.... 이런식으로 여러개 숨길수도 있음--%>
			<td><%= evo.getEmpno() %></td>
			<td><%= evo.getEname() %></td>
			<td><%= evo.getJob() %></td>
			<td><%= evo.getMgr() %></td>
  ...
  $("tfoot button").on("click", function() {
	  // ex01_02.jsp?empno=1111&empno=2222....이런 형식으로
	 let empnos=[];
	 $("tbody :checkbox:checked").each(function(i,elem){
		  
		 //jq 메서드 중 data()
		  let empno = $(elem).data("empno");  // 이러면 data-empno의 속성값 얻어옴
		  empnos.push(empno);
